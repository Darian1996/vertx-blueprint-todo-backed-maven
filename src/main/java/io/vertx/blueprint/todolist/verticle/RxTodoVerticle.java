...
public class RxTodoVerticle extends RestfulApiVerticle {
    ...
    public void start(Future<Void> startFuture) throws Exception {
        ...
        initService()
                ...
                .doOnComplete(() -> {
                    System.out.println("test网站 : http://" + host + ":" + port + "/index.html?http://" + host + ":" + port + "/todos");
                })
                .doOnComplete(() -> {
                    System.out.println("演示网站： http://www.todobackend.com/client/index.html?http://" + host + ":" + port + "/todos#/");
                })
    ...
    private Completable initService() {
        ...
        if ("redis".equals(serviceType)) {
            RedisOptions config = new RedisOptions()
                    ...
                    .setAuth(config().getString("redis.auth"));
            service = new RedisTodoService(vertx, config);

        } else {
            JsonObject JDBCConfig = new JsonObject();

            JDBCConfig.put("url", config().getString("url"));
            JDBCConfig.put("driver_class", config().getString("driver_class"));
            JDBCConfig.put("user", config().getString("user"));
            JDBCConfig.put("password", config().getString("password"));

            JDBCConfig.put("initial_pool_size", config().getInteger("initial_pool_size"));
            JDBCConfig.put("min_pool_size", config().getInteger("min_pool_size"));
            JDBCConfig.put("max_pool_size", config().getInteger("max_pool_size"));
